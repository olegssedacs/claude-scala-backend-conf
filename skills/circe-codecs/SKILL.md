...
- **NEVER skip the test** — codec regressions are silent at compile time but cause runtime deserialization failures when reading from the PostgreSQL event journal (`journal` schema) or view snapshots. A `verifyJson` test catches these before they reach production.
